...
) : PosControllerBase
{
    ...
    public async Task<ActionResult<BillResponse>> CreateBill(
        Guid restaurant_id, short branch_id, BillRequest body)
    {
        var result = await billService.CreateBill(
            BillResponse.Projection, new(
                new(restaurant_id, branch_id, body.table_id),
        ...
        return CreatedAtAction(
            nameof(GetBill),
            new { restaurant_id, branch_id, bill_id = key.Id },
            response);
    }
    ...
    public async Task<ActionResult> UpdateBill(
        Guid restaurant_id, short branch_id, Guid bill_id, BillRequest body)
    {
    ...
    public async Task<ActionResult<ICollection<OrderingPortalResponse>>> ListPortals(
        Guid restaurant_id, short branch_id, Guid bill_id)
        // [FromQuery] bool? is_expired)
    {
        Expression<Func<OrderingPortal, bool>> predicate = e =>
            e.BillId == bill_id &&
            e.Bill.RestaurantId == restaurant_id &&
            e.Bill.BranchId == branch_id;

        return await orderingPortalService.ListPortals(
            OrderingPortalResponse.Projection, predicate);
    }
        ...
        Guid bill_id, OrderingPortalRequest body)
    {
        ...
        return CreatedAtAction(
            nameof(GetPortal),
            new { restaurant_id, branch_id, bill_id, portal_id = key.Id },
            response);
    }
    ...
    public async Task<ActionResult<OrderingPortalResponse>> GetPortal(
        Guid restaurant_id, short branch_id, Guid bill_id, Guid portal_id)
    {
    ...
    public async Task<ActionResult> UpdatePortal(
        Guid restaurant_id, short branch_id,
        Guid bill_id, Guid portal_id, OrderingPortalRequest body)
    {
