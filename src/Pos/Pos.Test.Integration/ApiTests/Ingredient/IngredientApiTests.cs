...
public class IngredientApiTests(SharedAppFixture fixture) : SharedAppTestsBase(fixture)
{
    ...
    [Fact]
    public async Task List_Single_Ingredient_Succeed()
    {
        using var builder = CreateTestSeeder();
        ...
        var ingredient = await builder.SeedIngredient(restaurant);
        var stockTransaction = await builder.SeedStockTransaction(branch, ingredient);

        await builder.Commit();
        await Authenticate(owner);

        var response = await _client.GetAsync($"s/restaurants/{restaurant.Id}/ingredients", TestContext.Current.CancellationToken);
        var content = await response.Content.ReadAsStringAsync(TestContext.Current.CancellationToken);
        response.StatusCode.Should().Be(HttpStatusCode.OK, content);

        var responseBody = await response.Content.ReadFromJsonAsync<ICollection<SingleIngredientResponse>>(JsonSerializerOptions, TestContext.Current.CancellationToken);
        responseBody.Should().NotBeNull();
    }
}